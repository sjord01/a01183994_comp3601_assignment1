...
public class CustomerReport {
    ...
    private static class ReportColumn {
        ...
        static final List<ReportColumn> COLUMNS = List.of(
            new ReportColumn("ID", ID_WIDTH, Customer::getId),
            new ReportColumn("FIRST NAME", NAME_WIDTH, Customer::getFirstName),
            new ReportColumn("LAST NAME", NAME_WIDTH, Customer::getLastName),
            new ReportColumn("STREET", STREET_WIDTH, customer -> CustomerReportUtils.CustomerFormatter.truncateAndReplace(customer.getStreetName(), STREET_WIDTH)),
            ...
            new ReportColumn("PHONE", PHONE_WIDTH, customer -> CustomerReportUtils.CustomerFormatter.formatPhoneNumber(customer.getPhoneNumber())),
            new ReportColumn("EMAIL", EMAIL_WIDTH, Customer::getEmail),
            new ReportColumn("JOIN DATE", JOIN_DATE_WIDTH, customer -> customer.getJoinDate().format(DateTimeFormatter.ISO_LOCAL_DATE))
        );
    ...
    private static String generateReportContent(List<Customer> customers) {
        ...
        IntStream.range(0, customers.size())
            .mapToObj(i -> formatCustomer(customers.get(i)))
    ...
    private static String formatCustomer(Customer customer) {
        String[] fields = ReportColumn.COLUMNS.stream()
            .map(col -> col.valueExtractor.apply(customer))
            .toArray(String[]::new);
